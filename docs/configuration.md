...
## Writing the Global Config

You have two options for populating `~/.axiom/config.toml`:

- **`axiom setup`** — a guided CLI wizard that walks through OpenRouter key, Docker readiness, default image readiness, and BitNet mode selection. It performs surgical line edits against the existing file, so only keys you actively change are touched and every other byte is preserved. This intentionally avoids round-tripping the full config struct, which would risk writing empty project fields that shadow valid global values (the Issue 09 / Issue 10 failure mode). See [CLI Reference § axiom setup](cli-reference.md#axiom-setup).
- **Hand-editing** — open `~/.axiom/config.toml` in your editor and add the sections you need. Safe for power users; use `axiom doctor` afterward to confirm the result.

Project config (`.axiom/config.toml`) is always hand-edited or generated by `axiom init`. Keep secrets out of project config per Architecture §29.4 — `openrouter_api_key` belongs in the global file only.
